...
class OnboxPagoController extends Controller
{
    ...
    public function showByMonthYear(ActivityMesRequest $request): JsonResponse
    {
        try {
            ...
            $result = $this->onboxPagoService->filterByMonthYear($year, $month);
            ...
            ], "Pagos de onbox para {$year}-{$month}.");
        } catch (\Exception $e) {
            return $this->errorResponse('Error al obtener los pagos de onbox por mes.', 500);
        }
    }
    ...
    public function showBySemana(ActivitySemanaRequest $request): JsonResponse
    {
        try {
            ...
            $registros = $this->onboxPagoService->filterByWeek($year, $week);
            ...
            ], "Pagos de onbox para la semana {$week} del año {$year}.");
        } catch (\Exception $e) {
            return $this->errorResponse('Error al obtener los pagos de onbox por semana.', 500);
        }
    }
