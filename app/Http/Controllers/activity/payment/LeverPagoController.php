...
use App\Service\activity\payment\LeverPagoService;
...
class LeverPagoController extends Controller
{
    ...
    public function showByMonthYear(ActivityMesRequest $request): JsonResponse
    {
        try {
            ...
            $result = $this->leverPagoService->filterByMonthYear($year, $month);
            ...
            ], "Pagos de lever para {$year}-{$month}.");
        } catch (\Exception $e) {
            return $this->errorResponse('Error al obtener los pagos de lever por mes.', 500);
        }
    }
    ...
    public function showBySemana(ActivitySemanaRequest $request): JsonResponse
    {
        try {
            ...
            $registros = $this->leverPagoService->filterByWeek($year, $week);
            ...
            ], "Pagos de lever para la semana {$week} del año {$year}.");
        } catch (\Exception $e) {
            return $this->errorResponse('Error al obtener los pagos de lever por semana.', 500);
        }
    }
