...
CREATE TABLE `0cc_pinpon_clientes` (
...
INSERT IGNORE INTO `1090024db3`.`0cc_pinpon_clientes` (
    `cedula`, 
    `nombre`, 
    `nacimiento`, 
    `sexo`, 
...
SELECT 
    ...
    -- 3. Extrae lo que va después del segundo '|' (si existe y no está vacío)
    CASE 
        WHEN LENGTH(`last_pay`) - LENGTH(REPLACE(`last_pay`, '|', '')) >= 2 
        THEN IF(TRIM(SUBSTRING_INDEX(`last_pay`, '|', -1)) = '', NULL, SUBSTRING_INDEX(`last_pay`, '|', -1))
        ELSE NULL 
    END AS `d`,
    
    `operador`
FROM `1090024db2`.`0cc_pinpon_clientes`
